 ...
 * @version 1.0
 */
public class t_score_year_leader extends BaseTest {
...
//搜索字段 kind， 使用 == 搜索
        ...
        //搜索字段 name， 使用like搜索
        CodeGeneratorQueryColumnDTO u_id = CodeGeneratorQueryColumnDTO.builder()
                .columnName("u_id")
                .sqlOperate(SqlOperateTypeEnum.EQUALS).build();
//搜索字段 kind， 使用 == 搜索
        //搜索字段 type， 使用==搜索
        ...
        CodeGeneratorQueryColumnDTO approval = CodeGeneratorQueryColumnDTO.builder()
                .columnName("approval")
                .sqlOperate(SqlOperateTypeEnum.EQUALS).build();
        List<CodeGeneratorQueryColumnDTO> queryColumnList = Lists.newArrayList(id,u_id,year,quarter,approval);
        CodeGeneratorDTO codeGenerator = CodeGeneratorDTO.builder()
                .author("周天颖")//class 注释作者
                .company("三格数维")//class注释公司名字
                .tableName("t_score_year_leader")//表名
                .tablePrefix("t_score")//表名前缀
                .basePackage("net.lab1024.smartadmin")//包名
                .modulePackage("business.sumscore")//业务子包名
